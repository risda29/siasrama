            ...
            <div class="card">
                <div class="card-body overflow-auto">
                    ...
                    <div class="d-flex justify-content-end align-items-center mb-3" style="margin-right: 50px;">
                        <a href="{{ url('data-ruangan') }}" class="btn btn-primary btn-sm">Tambah Data</a>
                    </div>
                    ...
                    <div class="datatable-wrapper datatable-loading no-footer sortable searchable fixed-columns">
                        ...
                        <div class="datatable-container">
                            <table class="table datatable datatable-table">
                                ...
                                <tbody>
                                    @foreach( $data_ruangan as $no=> $item)
                                    <tr>
                                        <td>{{ $no + 1 }}</td>
                                        ...
                                        <td>
                                           ...
                                           <form action="{{ url('ruangan_destroy'.$item->id_ruangan) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Kamu Yakin Menghapus Data Ruangan Ini?')">Hapus</button>
                                        </form>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="tambahRuanganModalLabel">Tambah Data Ruangan</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <form action="{{ url('data-ruangan') }}" method="POST">
                                        @csrf
                                        <div class="mb-3">
                    ...
                    <div class="modal fade" id="editRuanganModal{{ $item->id_ruangan }}" tabindex="-1" aria-labelledby="editRuanganModalLabel{{ $item->id_ruangan }}" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="editRuanganModalLabel{{ $item->id_ruangan }}">Edit Data Ruangan</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <form action="{{ url('data-ruangan'.$item->id_ruangan) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        @method('PUT')
                                        <div class="mb-3">
                                            <label for="nm_ruangan" class="form-label">Nama Ruangan</label>
...
<script>
    document.querySelector('.btn-primary').addEventListener('click', function(e) {
        ...
        new bootstrap.Modal(document.getElementById('tambahRuanganModal')).show();
    });
