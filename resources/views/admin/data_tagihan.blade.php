                            ...
                            <table class="table datatable datatable-table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @php
                                                ...
                                                $santri = $data_santri->where('id_santri', $item->santri_id)->first();
                                            @endphp
                                    ...
                                    <td>{{ $bulanList[$item->bulan] ?? $item->bulan }}</td>
                                        ...
                                        <td>
                                           {{-- Editttttttttt --}}
                                           <a href="#" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editTagihanModal{{ $item->id_tagihan }}">Edit</a>

                                           {{-- hapusssssssssss --}}
                                           <form action="{{ url('tagihan_destroy'.$item->id_tagihan) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Kamu Yakin Menghapus Data Tagihan Ini?')">Hapus</button>
                                     </form>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ url('data-tagihan') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="santri_id" id="santri_id" required>
                            ...
                            @foreach ($data_santri as $santri)
                                <option value="{{ $santri->id_santri }}">{{ $santri->nm_santri }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="jumlah" class="form-label">Jumlah</label>
                        <input type="number" class="form-control" id="jumlah" name="jumlah" value="600000" required>
                    </div>
                    <div class="mb-3">
                  ...
                  <div class="modal fade" id="editTagihanModal{{ $item->id_tagihan }}" tabindex="-1" aria-labelledby="editTagihanModalLabel{{ $item->id_tagihan }}" aria-hidden="true">
                      <div class="modal-dialog">
                          <div class="modal-content">
                              <div class="modal-header">
                                  <h5 class="modal-title" id="editTagihanModalLabel{{ $item->id_tagihan }}">Edit Data Ruangan</h5>
                                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                              </div>
                              ...
                              <div class="modal-body">
                                <form action="{{ url('data-tagihan') }}" method="POST">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" name="santri_id" id="edit_santri_id" required>
                                            <option value="">- Pilih Santri -</option>
                                            @foreach ($data_santri as $santri)
                                                <option value="{{ $santri->id_santri }}">{{ $santri->nm_santri }}</option>
                                            @endforeach
                                    ...
                                    <div class="mb-3">
                                        <label for="edit_tgl_jatuh_tempo" class="form-label">Tanggal Jatuh Tempo</label>
                                        <input type="date" class="form-control" id="edit_tgl_jatuh_tempo" name="tgl_jatuh_tempo">
                                    </div>
                                    <div class="text-end">
